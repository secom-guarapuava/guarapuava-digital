...
async function main(){
  ...
  try {
    ...
    console.error('Falha ao buscar o feed:', e.message, '— mantendo noticias.json atual.');
    process.exit(0);
  }
  const noticias = parseFeed(xml);
  if (!noticias.length) {
    console.error('Feed sem itens — mantendo noticias.json atual.');
    process.exit(0);
  }
  for (const n of noticias) n.img = await ogImage(n.url);
  const data = { fonte: FEED, atualizado: new Date().toISOString().slice(0,10), noticias };
  // não sobrescreve com algo idêntico (evita commit à toa)
  const novo = JSON.stringify(data, null, 2) + '\n';
  if (existsSync(OUT)) {
    try {
      ...
      if (igual) { console.log('Sem novidades.'); process.exit(0); }
    } catch {}
  }
  writeFileSync(OUT, novo);
  console.log('noticias.json atualizado com', noticias.length, 'notícias.');
}
